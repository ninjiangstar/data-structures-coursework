...
ProgramState::ProgramState(int numLines) : m_numLines( numLines )
{
	...
	if(this->numLines() > 1001){
		throw std::length_error("ERR: Program has too many lines.");
	}
}
...
int ProgramState::getVars(std::string name) const
{
	std::map<std::string,int>::const_iterator iter = this->vars.find(name);
...
{
	if(this->m_curr + 1 >= this->m_numLines){
		this->makeEnd();
		throw std::out_of_range("ERR: Program terminated without end statement.");
	} else this->m_curr++;
}
